...
const { createSourceFile, ScriptTarget: { ES3, ES5, Latest }, ScriptKind: { TS }, SyntaxKind: { TypeAliasDeclaration, TypeAssertionExpression, AsExpression, HeritageClause, Identifier, FunctionExpression, FunctionDeclaration, ClassDeclaration, ClassExpression, ArrowFunction, MethodDeclaration, GetAccessor, SetAccessor, TypeParameter, VariableDeclaration, PropertyDeclaration, Parameter, QuestionToken, CallExpression, NewExpression, IntersectionType, TypeReference, LiteralType, UnionType, TupleType, StringKeyword, NumberKeyword, SymbolKeyword, BooleanKeyword, FunctionType, TypeLiteral, NullKeyword, NeverKeyword, AnyKeyword, ObjectKeyword, TypeOperator, BigIntKeyword, UndefinedKeyword, VoidKeyword, ArrayType, FirstTypeNode, ConditionalType, InterfaceDeclaration, EnumDeclaration, DeclareKeyword, Block, EndOfFileToken, NonNullExpression, UnknownKeyword, IndexedAccessType, TypeQuery, ModuleDeclaration, }, forEachChild, } = require('typescript');
const S = /\S/g;
...
function transpileModule(input, esv) {
    try {
        return {
            outputText: from(createSourceFile('', ts = input, esv
                ? esv === 3 ? ES3
                    : esv === 5 ? ES5
...
function from(node) {
    ...
    forEachChild(node, function (child) {
        if (!firstChild && ts_index === child.pos) {
            firstChild = child;
        }
    });
    if (firstChild && firstChild.kind === DeclareKeyword) {
        return remove(ts.slice(node.pos, node.end));
    }
    const es = [];
    switch (node.kind) {
                    ...
                    if (i) {
                        ...
                        forEachChild(child, function (child) {
                            if (ts_index !== child.pos) {
                                children.push(child.pos);
                            }
                            children.push(child);
                            ts_index = child.end;
                        });
                        es.push(children.length > 1 && children[0].kind === Identifier
        ...
        case NewExpression:
            ...
            forEachChild(node, function (child) {
                if (ts_index !== child.pos) {
                    children.push(ts.slice(ts_index, child.pos));
                }
                children.push(child);
                ts_index = child.end;
            });
            if (ts_index !== node.end) {
                children.push(ts.slice(ts_index, node.end));
            }
            if (node.kind === NewExpression) {
                es.push(children.shift());
            }
        ...
        case MethodDeclaration: {
            ...
            forEachChild(node, function (child) {
                if (declaration && child.kind === Block) {
                    declaration = false;
                }
            });
            if (declaration) {
        ...
        case ArrowFunction:
            ...
            forEachChild(node, function (child) {
                if (child.kind === TypeParameter) {
                    ...
                    if (gt) {
                        ...
                        es.push(ts.slice(ts_index, child.pos - 1) + ' ');
                    }
                    es.push(remove(ts.slice(child.pos, child.end)));
                }
                else {
            ...
            if (ts_index !== node.end) {
                es.push(ts.slice(ts_index, node.end));
            }
            break;
        ...
        case EndOfFileToken:
            if (node.pos !== node.end) {
                es.push(ts.slice(node.pos, node.end));
            }
            break;
        default:
            forEachChild(node, function (child) {
